return { -- Fuzzy Finder (files, lsp, etc)
	...
	config = function()
		...
		-- Setup fzf-lua with dropdown theme and icons
		fzf.setup()
		-- fzf.setup({
		-- 	winopts = {
		-- 		height = 0.85,
		...
		vim.keymap.set("n", "<leader>/", function()
			fzf.fzf_buffers({ prompt = "Buffers> " })
		...
		-- LSP pickers if lsp is attached
		vim.api.nvim_create_autocmd("LspAttach", {
			...
			callback = function(event)
				...
				set_keymap("grt", fzf.lsp_type_definitions, "[G]oto [T]ype Definition")
			end,
		})
	end,
}
